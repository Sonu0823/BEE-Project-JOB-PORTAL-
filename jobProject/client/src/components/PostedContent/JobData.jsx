        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
          {submittedData.map((job) => (
            <div key={job._id} className="rounded shadow p-4 w-full">
              ...
              <div className='bg-blue-500 mt-4 text-white-700 rounded-xl flex justify-center w-[40%]'>{job.employmentType}</div>
              <button className='bg-blue-500 mt-4 text-white-700 rounded-xl flex justify-center w-[40%]'>Apply Now </button>
</div>
              <div onClick={() => handleDelete(job._id)} className="bg-red-600 w-fit p-2  text-white cursor-pointer mt-2 hover:underline rounded-xl flex justify-center w-auto">
               Delete
              </div> 
            </div>
          ))}
